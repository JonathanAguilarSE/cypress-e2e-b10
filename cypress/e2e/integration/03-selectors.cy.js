...
describe("Cypress Selectors", () => {
    ...
    it("Practice Web Elements using - get and contains", () => {
        ...
        // This one tries to find element with 'HTML Elements' visual text inside the web element using .card class name
        cy.contains('.card', 'HTML Elements')

        // This will do the same thing as above locator in themore explicxcit way
        // It will look for cvard with 'HTML Elements' visible text only inside the class name
    ...
    it('Practice Web Elements - using find()', () => {
        ...
        // When we do this way, we try to locate the web element with 'Paragraph' text inside the div container
    ...
    it('Practice Web Elements using - Siblings, Parent and Child', () => {
        ...
        // next()       => locates th eimmediate next sibling of a web element
        cy.get('div').contains('Paragraph').next()

        // nextAll()    => locates the immediate next siublings of a web element
        cy.get('div').contains('Paragraph').nextAll()

        // prev()       => locates the immeediate previous siblng of a web element
        ...
        // Locate all the parents of a web element
        cy.get('#testing_paragraph').parents()
        
        // Locates until thge specific parent        
        cy.get('#testing_paragraph').parentsUntil('.HtmlElements_mainContainer__Fpn6M')

        cy.get('[data-identifier="Paragraphs"]').children()


        cy.get('#checkbox-button-group').next().find('div').children().find('input').parent().parent().parent().prev()
    })
}) 

...
describe("Assertions", () => {
    ...
    it("each() - interacting with multiple web elements", () => {
      ...
      it("Assertion Practices", () => {
         ...
         */
        let names = [ 'John', 'Doe' ]
        // cy.visit('https://techglobal-training.com/frontend').contains('HTML Elements')
        cy.get('input[id*=text]').each(($el, index) => {
            cy.wrap($el).should('be.enabled').type(names[index]).and('not.have.attr', 'required')
     ...
     */
        let dates = [ '01/01/1990', '02/02/1990'] 
        cy.get('input[id*=date_input]').each(($el, index) => {
     ...
     */
      it.only('Dropdowns', () => {
        let companies = [ 'Microsoft', 'Apple'] 
        cy.get('select[id^="company_dropdown"]').each(($el, index) => {
          cy.wrap($el).should('be.enabled').select(companies[index]).and('have.value', companies[index])

        })

        cy.get('#company_dropdown1, #company_dropdown2').each(($el, index) => {
          cy.wrap($el).should('be.enabled').select(companies[index])
          .find('option:selected').and('have.text', companies[index])
        })
        
      })
        /*
        // Burak examples to solve this task
